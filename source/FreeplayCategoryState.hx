...
class FreeplayCategoryState extends MusicBeatState
{
    ...
    override function create()
    {
        ...
        for (i in 0...selectionArray.length)
        {
            ...
            var selection = new FlxSprite(MobileUtil.fixX(x), y).loadGraphic(Paths.image('$assetLocation/$typeOfSel' + ((!isUnlocked) ? "Locked" : "")));
            selection.ID = i;
        ...
        for (i in 0...weekArray.length)
        {
            ...
            var weekAsset:FlxSprite = new FlxSprite(MobileUtil.fixX(x), y).loadGraphic(Paths.image('$assetLocation/week' + ((!isUnlocked) ? "Locked" : weekNum)));
    ...
    override function update(elapsed:Float)
    {
        if (!selectedSomething)
        {
            ...
            else
            {
                weekGroup.forEach(function(week:FlxSprite)
                {
                    if (TouchUtil.overlaps(week) && mouseMode)
                    {
                        if (subCatSelected != week.ID) 
                        {
                            if (ClientPrefs.haptics) Haptic.vibrateOneShot(0.05, 0.25, 0.5);
                            FlxG.sound.play(Paths.sound('scrollMenu'), 0.4);
                        }
