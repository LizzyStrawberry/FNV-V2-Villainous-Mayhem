...
class ResetScoreSubState extends MusicBeatSubstate
{
	...
	public function new(song:String, difficulty:Int, character:String, week:Int = -1)
	{
		...
		if(week == -1)
		{
			if(name != 'Mayhem Mode' && name != "Injection Mode")
				text.x += 60 * tooLong;
		}
		alphabetArray.push(text);
		text.alpha = 0;
		add(text);
		if(week == -1 && (name != 'Mayhem Mode' && name != "Injection Mode")) {
	...
	override function update(elapsed:Float)
	{
		...
		if(week == -1 && (song != 'Mayhem Mode' && song != "Injection Mode")) icon.alpha += elapsed * 2.5;
		...
		if(controls.BACK) {
			...
			if(onYes) {
				if(week == -1) {
					...
					{
						if (difficulty == 0)
							ClientPrefs.injectionEndScore = 0;
						else
							ClientPrefs.injectionVilEndScore = 0;
						ClientPrefs.saveSettings();
					}
					else
	...
	function updateOptions() {
		...
		if(week == -1 && (song != 'Mayhem Mode' && song != "Injection Mode")) icon.animation.curAnim.curFrame = confirmInt;
	}
}