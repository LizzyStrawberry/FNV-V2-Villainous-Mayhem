...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "pauseTween", function(tag:String) {
			PlayState.instance.modchartTweens.get(tag).active = false;
		});

		Lua_helper.add_callback(lua, "resumeTween", function(tag:String) {
			var tween:FlxTween;
			PlayState.instance.modchartTweens.get(tag).active = true;
		});
		...
		Lua_helper.add_callback(lua, "keyJustPressed", function(name:String) {
			...
			switch(name.toLowerCase()) {
				...
				case 'mayhem': key = PlayState.instance.getControl('MAYHEM');
				case 'charm1': key = PlayState.instance.getControl('CHARM1');
				case 'charm2': key = PlayState.instance.getControl('CHARM2');
				case 'charm3': key = PlayState.instance.getControl('CHARM3');
		...
		Lua_helper.add_callback(lua, "playDialogueVideo", function(videoFile:String) {
			#if VIDEOS_ALLOWED
			if(FileSystem.exists(Paths.video(videoFile))) {
		...
		if (ClientPrefs.shaders == true)
		{
			...
			Lua_helper.add_callback(lua, "addPulseEffect", function(camera:String,waveMult:Float = 0.1,waveSpeed:Float = 0.1,waveFrq:Float = 0.1,waveAmp:Float = 0.1) {
				PlayState.instance.addShaderToCamera(camera, new PulseEffect(waveMult,waveSpeed,waveFrq,waveAmp));			
			});
