...
class CreditsState extends MusicBeatState
{
	...
	override function create()
	{
		...
		creditsStuff = [ //Name - Icon name - Description - Quote - Link
			...
			['HQC', 				'hqc', 			'<G>Roles:<G>\n- Core Musician\n(Banger Music ong)', 														"'<C>Kevin<C>'", 												'https://x.com/hqcs_', 										'6bebfb'],
			...
			['Shiloh', 				'lanternAura', 	'<G>Roles:<G>\n- Core Musician\n(Coming in clutch fr)\n Chromatic Creator ', 								"'I'm <G>me<G>. me <G>be<G>. God damn. <G>I am.<G>'", 			'https://lanternaura.carrd.co/', 							'fc8901'],
	...
	override function update(elapsed:Float)
	{
		...
		if(!quitting)
		{
			if (TouchUtil.overlaps(arrowSelectorLeft)) FlxTween.tween(arrowSelectorLeft, {x: getarrowSelectorLeftX - 2}, 0.7, {ease: FlxEase.circOut, type: PERSIST});
			else FlxTween.tween(arrowSelectorLeft, {x: getarrowSelectorLeftX}, 0.7, {ease: FlxEase.circOut, type: PERSIST});

			if (TouchUtil.overlaps(arrowSelectorRight)) FlxTween.tween(arrowSelectorRight, {x: getarrowSelectorRightX + 2}, 0.7, {ease: FlxEase.circOut, type: PERSIST});
			else FlxTween.tween(arrowSelectorRight, {x: getarrowSelectorRightX}, 0.7, {ease: FlxEase.circOut, type: PERSIST});

			if (FlxG.keys.pressed.TAB || touchPad.buttonC.justPressed) MusicBeatState.switchState(new LegacyCreditsState());	
			...
			if (creditsSelected == 'CURRENT')
			{
				if(controls.UI_LEFT_P) changeCredits(-1, true, true);
				if(controls.UI_RIGHT_P) changeCredits(1, true, true);
				...
				if (TouchUtil.pressAction(icon) || controls.ACCEPT) CoolUtil.browserLoad(creditsStuff[curSelected][4]);
			}
			else if (creditsSelected == 'FORMER')
			{
				if(controls.UI_UP_P) changeCredits(-1, true, true);
				if(controls.UI_DOWN_P) changeCredits(1, true, true);

				if (controls.ACCEPT || TouchUtil.pressAction(grpOptions.members[curSelectedFormer])) CoolUtil.browserLoad(formerCreditsStuff[curSelectedFormer][3]);
			}

			if (!transitioning)
			{
				if (TouchUtil.pressAction(arrowSelectorLeft) || TouchUtil.pressAction(arrowSelectorRight))
				{
					if (creditsSelected == 'CURRENT') changeCreditsCategory('FORMER');
					else changeCreditsCategory('CURRENT');
					transitioning = true;
				}
			}
		}
	...
	function changeCredits(change:Int = 0, playSound:Bool = true, tween:Bool = true, ?allowHaptics:Bool = true)
	{
		...
		if (creditsSelected == 'CURRENT')
		{
			...
			if (curSelected > creditsStuff.length - 1) curSelected = 0;
	
			creditSprite.loadGraphic(Paths.image('credits/FNVDevs/credits-' + creditsStuff[curSelected][1]));
			if (curSelected == 4) creditSprite.scale.set(1.2, 1.2);
			else creditSprite.scale.set(1, 1);

			if (curSelected == 11) creditSprite.x = MobileUtil.fixX(358.5);
			else creditSprite.x = MobileUtil.fixX(383.5);
	...
	function getCurrentBGColor() {
		var bgColor:String;
		if (creditsSelected == 'FORMER') bgColor = formerCreditsStuff[curSelectedFormer][4];
		else bgColor = creditsStuff[curSelected][5];
	...
	function fixFormerCredits(change:Int = 0)
	{
		do 
		{
			...
			if (curSelectedFormer < 0) curSelectedFormer = formerCreditsStuff.length - 1;
			if (curSelectedFormer >= formerCreditsStuff.length) curSelectedFormer = 0;
		} 
		while(unselectableCheck(curSelectedFormer));
		...
		for (item in grpOptions.members)
		{
			...
			if(!unselectableCheck(bullShit - 1))
			{
				...
				switch(Math.abs(item.targetY))
				{
					case 0: FlxTween.tween(item, {alpha: 1, "scale.x": 1, "scale.y": 1}, 0.5, {ease: FlxEase.sineOut});
					case 1: FlxTween.tween(item, {alpha: 0.5, "scale.x": 0.75, "scale.y": 0.75}, 0.5, {ease: FlxEase.sineOut});
					default: FlxTween.tween(item, {alpha: 0, "scale.x": 0.5, "scale.y": 0.5}, 0.5, {ease: FlxEase.sineOut});
				}
			}
