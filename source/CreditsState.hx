...
class CreditsState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if(!quitting)
		{
			if (FlxG.mouse.overlaps(arrowSelectorLeft)) FlxTween.tween(arrowSelectorLeft, {x: getarrowSelectorLeftX - 2}, 0.7, {ease: FlxEase.circOut, type: PERSIST});
			else FlxTween.tween(arrowSelectorLeft, {x: getarrowSelectorLeftX}, 0.7, {ease: FlxEase.circOut, type: PERSIST});

			if (FlxG.mouse.overlaps(arrowSelectorRight)) FlxTween.tween(arrowSelectorRight, {x: getarrowSelectorRightX + 2}, 0.7, {ease: FlxEase.circOut, type: PERSIST});
			else FlxTween.tween(arrowSelectorRight, {x: getarrowSelectorRightX}, 0.7, {ease: FlxEase.circOut, type: PERSIST});

			if (FlxG.keys.pressed.TAB) MusicBeatState.switchState(new LegacyCreditsState());	
			...
			if (creditsSelected == 'CURRENT')
			{
				...
				if ((FlxG.mouse.overlaps(icon) && FlxG.mouse.justPressed) || controls.ACCEPT) CoolUtil.browserLoad(creditsStuff[curSelected][4]);
			}
			else if (creditsSelected == 'FORMER')
			{
				...
				if (controls.ACCEPT) CoolUtil.browserLoad(formerCreditsStuff[curSelectedFormer][3]);
			}

			if (!transitioning)
			{
				if ((FlxG.mouse.overlaps(arrowSelectorLeft) || FlxG.mouse.overlaps(arrowSelectorRight)) && FlxG.mouse.justPressed)
				{
	...
	function changeCredits(change:Int = 0, playSound:Bool = true, tween:Bool = true)
	{
		...
		if (creditsSelected == 'CURRENT')
		{
			...
			if (curSelected == 11) creditSprite.x = 358.5;
			else creditSprite.x = 383.5;
