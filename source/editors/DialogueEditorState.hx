...
class DialogueEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		defaultLine = {
			...
			charVisible: DEFAULT_VISIBILITY,
			speed: 0.05,
			sound: '',
			image: '',
			imageAlpha: 0.5,
			imageScale: 1,
			boxVisible: DEFAULT_BOX_VISIBILITY
		};
	...
	var characterVisibleCheckbox:FlxUICheckBox;
	...
	var imageAlphaStepper:FlxUINumericStepper;
	var imageScaleStepper:FlxUINumericStepper;
	var boxVisibleCheckbox:FlxUICheckBox;
	function addDialogueLineUI() {
		...
		characterVisibleCheckbox = new FlxUICheckBox(characterInputText.x + 120, characterInputText.y, null, null, "Character Visible", 200);
		characterVisibleCheckbox.callback = function()
		{
			updateTextBox();
			dialogueFile.dialogue[curSelected].charVisible = (characterVisibleCheckbox.checked ? 'No' : 'Yes');
		};
		...
		imageInputText = new FlxUIInputText(10, lineInputText.y + 35, 200, '', 8);
		blockPressWhileTypingOn.push(imageInputText);

		imageAlphaStepper = new FlxUINumericStepper(10, imageInputText.y + 40, 0.1, 0.05, 0, 1, 1);
		imageScaleStepper = new FlxUINumericStepper(130, imageInputText.y + 40, 0.05, 0.05, 0.1, 3, 2);

		boxVisibleCheckbox = new FlxUICheckBox(imageAlphaStepper.x, imageScaleStepper.y + 35, null, null, "Box Visible", 200);
		boxVisibleCheckbox.callback = function()
		{
			updateTextBox();
			dialogueFile.dialogue[curSelected].boxVisible = (boxVisibleCheckbox.checked ? 'No' : 'Yes');
		};

		var loadButton:FlxButton = new FlxButton(20, boxVisibleCheckbox.y + 25, "Load Dialogue", function() {
			loadDialogue();
		});
		...
		tab_group.add(new FlxText(10, imageInputText.y - 18, 0, 'Background Image:'));
		tab_group.add(new FlxText(10, imageAlphaStepper.y - 18, 0, 'Background Ocacity:'));
		tab_group.add(new FlxText(130, imageAlphaStepper.y - 18, 0, 'Background Scale:'));
		tab_group.add(characterInputText);
		tab_group.add(characterVisibleCheckbox);
		tab_group.add(angryCheckbox);
		tab_group.add(speedStepper);
		tab_group.add(soundInputText);
		tab_group.add(lineInputText);
		tab_group.add(imageInputText);
		tab_group.add(imageAlphaStepper);
		tab_group.add(imageScaleStepper);
		tab_group.add(boxVisibleCheckbox);
	...
	function copyDefaultLine():DialogueLine {
		var copyLine:DialogueLine = {
			...
			charVisible: defaultLine.charVisible,
			speed: defaultLine.speed,
			sound: '',
			image: defaultLine.image,
			imageAlpha: defaultLine.imageAlpha,
			imageScale: defaultLine.imageScale,
			boxVisible: defaultLine.boxVisible
		};
		return copyLine;
	}
	...
	function updateTextBox() {
		...
		var isVisible:Bool = characterVisibleCheckbox.checked;
		var visible:String = isVisible ? 'No' : 'Yes';
		var isBoxVisible:Bool = boxVisibleCheckbox.checked;
		var boxvisibility:String = isBoxVisible ? 'No' : 'Yes';
	...
	private static var DEFAULT_VISIBILITY:String = "Yes";
	private static var DEFAULT_BOX_VISIBILITY:String = "Yes";
	...
	function reloadImage(defaultImage:Bool = false) {
		var textToImage:String = imageInputText.text;
		if(textToImage == null || textToImage.length < 1) textToImage = ' ';
		if (defaultImage == true)
		{
			bgFade.makeGraphic(1280, 720, FlxColor.WHITE);
			bgFade.screenCenter(XY);
			bgFade.scale.set(dialogueFile.dialogue[curSelected].imageScale, dialogueFile.dialogue[curSelected].imageScale);
		}
			
		
		if (textToImage == ' ' || textToImage == null)
		{
			bgFade.makeGraphic(1280, 720, FlxColor.WHITE);
			bgFade.screenCenter(XY);
			bgFade.scale.set(dialogueFile.dialogue[curSelected].imageScale, dialogueFile.dialogue[curSelected].imageScale);
		...
		{
			bgFade.loadGraphic(Paths.image('dialogueBackground/' + textToImage));
			bgFade.screenCenter(XY);
			bgFade.scale.set(dialogueFile.dialogue[curSelected].imageScale, dialogueFile.dialogue[curSelected].imageScale);
		}
		
		bgFade.alpha = dialogueFile.dialogue[curSelected].imageAlpha;

		#if desktop
		// Updating Discord Rich Presence
		var rpcText:String = imageInputText.text;
		if(rpcText == null || rpcText.length < 1) rpcText = '(Empty)';
		if(rpcText.length < 3) rpcText += '   '; //Fixes a bug on RPC that triggers an error when the text is too short
		DiscordClient.changePresence("Dialogue Editor", rpcText);
		#end
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			{
				dialogueFile.dialogue[curSelected].image = imageInputText.text;
				reloadImage(false);
			}
		...
		} else if(id == FlxUINumericStepper.CHANGE_EVENT)
		{
			...
			if (sender == imageAlphaStepper) {
				dialogueFile.dialogue[curSelected].imageAlpha = imageAlphaStepper.value;
				if(Math.isNaN(dialogueFile.dialogue[curSelected].imageAlpha) || dialogueFile.dialogue[curSelected].imageAlpha == null || dialogueFile.dialogue[curSelected].imageAlpha < 0.001) {
					dialogueFile.dialogue[curSelected].imageAlpha = 0.0;
				}
				reloadImage(false);
			}

			if (sender == imageScaleStepper) {
				dialogueFile.dialogue[curSelected].imageScale = imageScaleStepper.value;
				if(Math.isNaN(dialogueFile.dialogue[curSelected].imageScale) || dialogueFile.dialogue[curSelected].imageAlpha == null || dialogueFile.dialogue[curSelected].imageScale < 0.001) {
					dialogueFile.dialogue[curSelected].imageScale = 0.1;
				}
				reloadImage(false);
			}
		}
	...
	function changeText(add:Int = 0) {
		...
		characterVisibleCheckbox.checked = (curDialogue.charVisible == 'No');
		lineInputText.text = curDialogue.text;
		angryCheckbox.checked = (curDialogue.boxState == 'angry');
		speedStepper.value = curDialogue.speed;
		imageInputText.text = curDialogue.image;
		imageAlphaStepper.value = curDialogue.imageAlpha;
		imageScaleStepper.value = curDialogue.imageScale;
		boxVisibleCheckbox.checked = (curDialogue.boxVisible == 'No');
