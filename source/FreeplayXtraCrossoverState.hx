...
class FreeplayXtraCrossoverState extends MusicBeatState
{
	var songs:Array<SongXtraCrossoverMetadata> = [];
	...
	override function create()
	{
		...
		DiscordClient.changePresence("In XTRA CROSSOVER Freeplay Mode", null);
		#end
		...
		if (ClientPrefs.ourpleFound == true && ClientPrefs.ourplePlayed == false)
			addSong('VGuy', 3, 'mystery', FlxColor.fromRGB(115, 13, 255));
		if (ClientPrefs.ourpleFound == true && ClientPrefs.ourplePlayed == true)
			addSong('VGuy', 3, 'ourple', FlxColor.fromRGB(115, 13, 255));

		if (ClientPrefs.kyuFound == true && ClientPrefs.kyuPlayed == false)
			addSong('Fast Food Therapy', 3, 'face', FlxColor.fromRGB(255, 239, 11));
		if (ClientPrefs.kyuFound == true && ClientPrefs.kyuPlayed == true)
			addSong('Fast Food Therapy', 3, 'kyuFA', FlxColor.fromRGB(255, 239, 11));

		if (ClientPrefs.tacticalMishapFound == true && ClientPrefs.tacticalMishapPlayed == false)
			addSong('Tactical Mishap', 3, 'mystery', FlxColor.fromRGB(255, 73, 155));
		if (ClientPrefs.tacticalMishapFound == true && ClientPrefs.tacticalMishapPlayed == true)
			addSong('Tactical Mishap', 3, 'TC', FlxColor.fromRGB(255, 73, 155));

		if (ClientPrefs.breacherFound == true && ClientPrefs.breacherPlayed == false)
		...
		if (ClientPrefs.ccFound == true && ClientPrefs.ccPlayed == true)
			addSong('Concert Chaos', 3, 'lilyCC', FlxColor.fromRGB(155,0,206));
		...
		for (i in 0...songs.length)
		{
			...
			if (songText.text == 'VGuy' && ClientPrefs.ourplePlayed == false)
				songText.text = '????';

			if (songText.text == 'Tactical Mishap' && ClientPrefs.tacticalMishapPlayed == false)
				songText.text = '???????? ??????';
		
			if (songText.text == 'Fast Food Therapy' && ClientPrefs.kyuPlayed == false)
				songText.text = '???? ???? ???????';
	...
	{
		songs.push(new SongXtraCrossoverMetadata(songName, weekNum, songCharacter, color));
	}
	...
	function songSelector()
	{
		switch(songs[curSelected].songName)
		{
			case 'Tactical Mishap' | "???????? ??????":
			{
				...
				selectionText.text = "Tactical Mishap";
	
				if (ClientPrefs.tacticalMishapPlayed == false)
				{
					selectionText.text = "???????? ??????";
			...
			case 'VGuy' | "????":
			{
				...
				selectionText.text = "VGuy";
	
				if (ClientPrefs.ourplePlayed == false)
				{
					selectionText.text = "????";
					lockedSelection.alpha = 1;
				}
			...
			case 'Fast Food Therapy' | "???? ???? ???????":
			{
				...
				selectionText.text = "Fast Food Therapy";

				if (ClientPrefs.kyuPlayed == false)
				{
					selectionText.text = "???? ???? ???????";
					lockedSelection.alpha = 1;
				}
			...
			case 'Concert Chaos' | "??????? ?????":
			{
				...
				selectionText.text = "Concert Chaos";

				if (ClientPrefs.ccPlayed == false)
				{
					selectionText.text = "??????? ?????";
					lockedSelection.alpha = 1;
				}
...
class SongXtraCrossoverMetadata
{
