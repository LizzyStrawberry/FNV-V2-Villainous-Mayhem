...
class CharSelector extends MusicBeatState{
    ...
    override function update(elapsed:Float)
    {
        ...
        if (!alreadySelected)
        {
            if (TouchUtil.overlaps(icon)) FlxTween.tween(icon, {y: getIconY - 2}, 0.7, {ease: FlxEase.circOut, type: PERSIST});
            else FlxTween.tween(icon, {y: getIconY}, 0.7, {ease: FlxEase.circOut, type: PERSIST});

            if (TouchUtil.overlaps(arrowSelectorLeft)) FlxTween.tween(arrowSelectorLeft, {x: getLeftArrowX - 2}, 0.7, {ease: FlxEase.circOut, type: PERSIST});
			else FlxTween.tween(arrowSelectorLeft, {x: getLeftArrowX}, 0.7, {ease: FlxEase.circOut, type: PERSIST});
	
			if (TouchUtil.overlaps(arrowSelectorRight)) FlxTween.tween(arrowSelectorRight, {x: getRightArrowX + 2}, 0.7, {ease: FlxEase.circOut, type: PERSIST});
