local hudThings = {'watermark', 'healthBar', 'healthBarBG', 'iconP1', 'iconP2', "iconGF", 'scoreTxt'}
...
function onSongStart()
	...
	doTweenAlpha(hudThings[6], hudThings[6], 1, 14 / playbackRate, 'cubeInOut')
...
function onUpdate()
	...
	if curStep == 2372 and not videoPlayed then
		startVideo("Shucks Cutscene", false, true, false, true) -- Umskippable, for Mid Song, not loopable, play on load
		setProperty('videoCutscene.alpha', 0)
		setObjectCamera("videoCutscene", "hud")
		setProperty("videoCutscene.videoSprite.volumeAdjust", 0)
		setObjectOrder("videoCutscene", 0)
		videoPlayed = true
	end
	if curStep == 2380 then
		hudThings = {'watermark', 'healthBar', 'healthBarBG', 'iconP1', 'iconP2', "iconGF", 'scoreTxt', 'timeBar', 'timeBarBG', 'timeTxt'}
		for i = 1, #(hudThings) do
