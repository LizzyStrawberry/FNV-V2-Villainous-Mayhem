...
function onCountdownTick(counter)
	if mechanics then
		if counter == 0 then
			doTweenY('barBackStart', 'barBack.scale', 0.001, 0.5, 'sineOut')
		end
	end
end
...
function opponentNoteHit(id, direction, noteType, isSustainNote, noteData)
	if mechanics and getPropertyFromClass('ClientPrefs', 'buff3Active') == false then
		if not isMayhemMode and getProperty('health') > 0.4 and difficulty == 1 then
...
function onTimerCompleted(tag)
		...
		if tag == 'TailAttack' then
			runTimer('AnimatedTimerPlay', 3)
			runTimer('spaceHit', 2.8)
...
function miss()
		...
		triggerEvent('Play Animation', 'stunned', 'bf')
