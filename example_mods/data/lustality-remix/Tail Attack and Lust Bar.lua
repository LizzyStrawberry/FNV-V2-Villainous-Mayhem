-- Tail Attack / Lust Notes
local randNum = 0
local pressedKey = false --stealing some of my code lmaooaoaodjjkfhwojuvf
local missedKey = false
local canDodge = false
local timerCompleted = false
local turnedTimerOn = false
local warningOn = false
local gotHit = false
local songStarted = false
local gotAttacked = 0
local LustCount = 0

-- Attack
local attackNum = 10

function onCreate()
	if mechanics then
		precacheSound('GFAttack')
		precacheSound('GFDodge')
		precacheSound('DirectHit')
		
		makeAnimatedLuaSprite('time2', 'effects/timer', 560 , 40)
		addAnimationByPrefix('time2', 'Stunned', 'timer rundown0', 24, false)
		setProperty('time2.alpha', 0)
		scaleObject('time2', 0.5, 0.5)
		setObjectCamera('time2', 'hud')
		setScrollFactor('time2', 1, 1)
		addLuaSprite('time2', true)
		
		makeLuaSprite('dodgeNow', 'effects/dodge', 370, 170)
		setProperty('dodgeNow.alpha', 0)
		scaleObject('dodgeNow', 0.6, 0.6)
		setObjectCamera('dodgeNow', 'hud')
		setScrollFactor('dodgeNow', 1, 1)
		addLuaSprite('dodgeNow', true)
		
		makeLuaSprite('barBack', 'effects/barBack', 1100, 170)
		setObjectCamera('barBack', 'hud')
		setProperty('barBack.origin.y', 365)
		addLuaSprite('barBack', true)
		
		makeLuaSprite('bar', 'effects/bar', 1100, 170)
		setObjectCamera('bar', 'hud')
		addLuaSprite('bar', true)
		
		for i = 1, 3 do
		makeLuaSprite('counterTime'..i, 'effects/counter-'..i, getProperty('bar.x'), 540)
		setObjectCamera('counterTime'..i, 'hud')
		end
		
		if difficulty == 0 then
			addLuaSprite('counterTime3', true)
		end
		if difficulty >= 1 then
			addLuaSprite('counterTime1', true)
		end
		
		makeLuaText('warning', 'Get Ready!', 600, 345, 200)
		setTextSize('warning', 50)
		setProperty('warning.alpha', 0)
		addLuaText('warning')
		
		if downscroll then
			setProperty('time2.y', 540)
			setProperty('warning.y', 400)
		end
		
		-- Attack
		makeLuaSprite('attackButton', 'effects/attack', getProperty('bar.x') - 1120, 380)
		scaleObject('attackButton', 0.8, 0.8)
		setProperty('attackButton.alpha', 0.7)
		setBlendMode('attackButton', 'overlay')
		setObjectCamera('attackButton', 'hud')
		addLuaSprite('attackButton')
		
		makeLuaText('attackText', attackNum, 600, getProperty('attackButton.x') - 228, getProperty('attackButton.y') + 16)
		setTextSize('attackText', 32)
		setProperty('attackText.alpha', 1)
		addLuaText('attackText')
	end
end

function onSongStart()
	if mechanics then
		if difficulty == 0 then
			randNum = getRandomInt(20, 25)
		end
		if difficulty == 1 then
			randNum = getRandomInt(10, 15)
		end
		if difficulty == 2 then
			randNum = getRandomInt(5, 10)
		end
		
		-- Attack
		attackNum = 10
		runTimer('reduceAttackNumber', 1)
		
		songStarted = true
	end
end

function onCountdownTick(counter)
	if mechanics then
		if counter == 0 then
			doTweenY('barBackStart', 'barBack.scale', 0.001, 1.0, 'cubeInOut')
		end
	end
end

function noteMiss(id, direction, noteType, isSustainNote)
	if mechanics then
		if noteType == 'Lust Notes' then
			LustCount = LustCount + 1;
			if difficulty <= 1 then
				if not isMayhemMode then
					setProperty('health', health - 0.08)
				else
					setProperty('health', health - 1)
				end
				if LustCount < 10 then
					setProperty('barBack.scale.y', bar + 0.1)
				end
				if LustCount == 10 then
					setProperty('barBack.scale.y', 1)
				end
			end
			if difficulty == 2 then
				if not isMayhemMode then
					setProperty('health', health - 0.08)
				else
					setProperty('health', health - 1)
				end
				if LustCount < 5 then
					setProperty('barBack.scale.y', bar + 0.2)
				end
				if LustCount == 5 then
					setProperty('barBack.scale.y', 1)
				end
			end
		end
	end
end

function onUpdate()
	if mechanics then
		bar = getProperty('barBack.scale.y')
		health = getProperty('health') --uwu
		-- Tail Attack Mechanic
		if not timerCompleted and songStarted == true and not turnedTimerOn then
			--debugPrint('Setting Timer Up! Random Number is: ' ..randNum)
			
			setProperty('dodgeNow.alpha', 0)
			setTextColor('warning', 'ffffff')
			setTextString('warning', 'Get Ready!')
			setProperty('warning.alpha', 0)
			setProperty('time2.alpha', 0)
			runTimer('TailAttack', randNum)
			
			turnedTimerOn = true
			warningOn = false
		end
		if canDodge then
			if keyJustPressed('dodge') then
				canDodge = false
			else 
				canDodge = true
			end
		end
		
		if difficulty <= 1 then
			if LustCount >= 10 and songStarted then
				if curStep % 8 == 0 then
					doTweenColor('WarningTime', 'barBack', 'FF0000', 0.4, 'cubeInOut')
				end
				if curStep % 8 == 4 then
					doTweenColor('WarningTime', 'barBack', 'ff9f9f', 0.4, 'cubeInOut')
				end
			end
			if difficulty == 1 and LustCount == 11 then	
				setProperty('health', 0)
			end
			if difficulty == 0 and LustCount == 16 then
				setProperty('health', 0)
			end
		end
		if difficulty == 2 then
			if LustCount >= 5 and songStarted then
				if curStep % 8 == 0 then
					doTweenColor('WarningTime', 'barBack', 'FF0000', 0.4, 'cubeInOut')
				end
				if curStep % 8 == 4 then
					doTweenColor('WarningTime', 'barBack', 'ff9f9f', 0.4, 'cubeInOut')
				end
			end
			if LustCount == 6 then	
				setProperty('health', 0)
			end
		end
		--Attack Button
		if attackNum > 0 then
			setTextString('attackText', attackNum)
		else
			setProperty('attackText.visible', false)
			setProperty('attackButton.alpha', 1)
			setBlendMode('attackButton', 'normal')
		end
		
		if keyJustPressed('attack') and attackNum == 0 then
			setPropertyFromClass('PlayState', 'choseAttack', true)
			triggerEvent('Play Animation', 'attack', 'bf')
			runTimer('fixAttack', 0.8)
			runTimer('KianaDodgeDone', 1)
			attackNum = 10
			runTimer('reduceAttackNumber', 1)
			
			setBlendMode('attackButton', 'overlay')
			setProperty('attackButton.alpha', 0.7)
			setProperty('attackText.visible', true)
		end
	end
end

function onTimerCompleted(tag)
	-- GF Attack
	if tag == 'reduceAttackNumber' and attackNum > 0 then
		attackNum = attackNum - 1
		runTimer('reduceAttackNumber', 1)
	end
	
	if tag == 'fixAttack' then
		triggerEvent('Play Animation', 'dodge', 'dad')
		if difficulty <= 1 then
			if not isMayhemMode then
				setProperty('health', health + 0.15)
			else
				setProperty('health', health + 1)
			end
			if LustCount > 0 then
				LustCount = LustCount - 1
			end
			if LustCount >= 10 then
				setProperty('barBack.scale.y', 1)
			end
			if LustCount < 10 then
				setProperty('barBack.scale.y', bar - 0.1)
				doTweenColor('WarningTime', 'barBack', 'FFFFFF', 0.4, 'cubeInOut')
			end
		end
		if difficulty == 2 then
			if not isMayhemMode then
				setProperty('health', health + 0.20)
			else
				setProperty('health', health + 1)
			end
			if LustCount > 0 then
				LustCount = LustCount - 1
			end
			if LustCount >= 5 then
				setProperty('barBack.scale.y', 1)
			end
			if LustCount < 5 then
				setProperty('barBack.scale.y', bar - 0.2)
				doTweenColor('WarningTime', 'barBack', 'FFFFFF', 0.4, 'cubeInOut')
			end
		end
		if getProperty('barBack.scale.y') <= 0 then
			setProperty('barBack.scale.y', 0.001)
		end
		playSound('GFAttack')
	end
	
	if tag == 'fixDodge' then
		triggerEvent('Play Animation', 'dodge', 'dad')
	end
	
	if tag == 'KianaDodgeDone' then
		setPropertyFromClass('PlayState', 'choseAttack', false)
	end
	
	--once the timer is completed / Tail Attack
	if tag == 'TailAttack' then
		if difficulty <= 1 then
			runTimer('AnimatedTimerPlay', 3 / playbackRate)
			runTimer('spaceHit', 2.7 / playbackRate)
				
			doTweenAlpha('Timer2GoHello', 'time2', 1, 0.2, 'linear')
			objectPlayAnimation('time2','Stunned', true)
				
			if curBeat >= 0 and curBeat <= 295 then
				triggerEvent('Change Character', 'dad', 'kianaAttack')
				triggerEvent('Play Animation', 'tail attack', 'dad')
			end
			
			timerCompleted = true	
			warningOn = true
			--debugPrint('Timer Ended! Watch Out for her attack!')
			--debugPrint(timerCompleted)
		end
		if difficulty == 2 then
			runTimer('AnimatedTimerPlay', 1.5 / playbackRate)
			runTimer('spaceHit', 0.4 / playbackRate)
		end
	end
	if tag == 'spaceHit' then
		--debugPrint("Press Now!")
		if difficulty <= 1 then
			setProperty('dodgeNow.alpha', 1)
		end
		if difficulty == 2 then
			doTweenAlpha('dodgeNow', 'dodgeNow', 1, 0.2, 'circOut')
		end
		setProperty('warning.alpha', 0)
			
		warningOn = false
		canDodge = true
	end
	if tag == 'AnimatedTimerPlay' then
		if curBeat >= 0 and curBeat <= 143 then
			triggerEvent('Change Character', 'dad', 'kiana')
		end
		if curBeat >= 144 and curBeat <= 295 then
			triggerEvent('Change Character', 'dad', 'kianaPhase2')
		end
		if botPlay or getPropertyFromClass('ClientPrefs', 'autoCharm') == 1 then
			hit()
			doTweenAlpha('Timer2GoByeBye', 'time2', 0, 0.3, 'linear')
		elseif not botPlay then
			if not canDodge then	
				hit()
				doTweenAlpha('Timer2GoByeBye', 'time2', 0, 0.3, 'linear')
			elseif canDodge then
				miss()
			end
		end
	end
end

function onEndSong()
	cancelTimer('AnimatedTimerPlay')
	cancelTimer('TailAttack')
	cancelTimer('spaceHit')
	songStarted = false
end

function onStepHit()
	if warningOn then
		if curStep % 8 == 0 then
			setProperty('warning.alpha', 1)
		end
		if curStep % 8 == 4 then
			setProperty('warning.alpha', 0)
		end
	end
end

function miss()
	playSound('DirectHit')
		
	cameraFlash('game', 'ff0000', 0.2, false)
	triggerEvent('Play Animation', 'singUPmiss', 'bf')
		
	if difficulty == 0 then
		randNum = getRandomInt(20, 25)
	end
	if difficulty >= 1 then
		setProperty('health', 0) --here, if the difficulty is on villainous mode, you die.
	end
			
	gotHit = true
	timerCompleted = false
	turnedTimerOn = false
end

function onUpdatePost() --Go here after miss()
	if gotHit then
		gotAttacked = gotAttacked + 1
		if difficulty == 0 then
			if gotAttacked == 1 then
				addLuaSprite('counterTime2', true)
				removeLuaSprite('counterTime3', true)
			end
			if gotAttacked == 2 then
				addLuaSprite('counterTime1', true)
				removeLuaSprite('counterTime2', true)
			end
			if gotAttacked == 3 then
				setProperty('health', 0)
			end
		end
		gotHit = false
	end
end

function hit()
	playSound('clickText')
	playSound('GFDodge')
		
	cameraFlash('game', 'cd8b8b', 0.2, false)
	
	if difficulty <= 1 then
		playSound('GFAttack')
		setPropertyFromClass('PlayState', 'choseAttack', true)
		if boyfriendName == 'playablegf' then
			triggerEvent('Play Animation', 'dodgenormal', 'bf')
		else
			triggerEvent('Play Animation', 'dodge', 'bf')
		end
		runTimer('fixDodge', 0.4)
		runTimer('KianaDodgeDone', 0.6)
	end
	if difficulty == 2 then
		triggerEvent('Play Animation', 'dodge', 'bf')
	end
		
	if difficulty == 0 then
		randNum = getRandomInt(20, 25)
	end
	if difficulty == 1 then
		randNum = getRandomInt(10, 15)
	end
	if difficulty == 2 then
		randNum = getRandomInt(5, 10)
	end
		
	timerCompleted = false
	turnedTimerOn = false
end

function onDestroy()
	setPropertyFromClass('PlayState', 'choseAttack', false)
	return Function_Continue
end