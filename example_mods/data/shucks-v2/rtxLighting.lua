...
function onCreatePost()
    ...
    setSpriteShader('fg', 'RTXLighting')
    setSpriteShader('bg', 'RTXLighting')
    setSpriteShader('bf', 'RTXLighting')
    setSpriteShader('ded', 'RTXLighting')
...
function onStepHit()
	if curStep == 704 and songName == 'Shucks V2' then
		if not addedShader and shadersEnabled then
			runHaxeCode([[
				...
				for (note in game.unspawnNotes) note.shader = game.createRuntimeShader("RTXLighting");
				for (note in game.notes) note.shader = game.createRuntimeShader("RTXLighting");
			]])
...
local shit = {'boyfriend', 'gf', 'dad', 'bg', 'fg', 'bf'}
function updateShader()--update shader from slider stored values
	for i = 1, #(shit) do
		setShaderFloatArray(shit[i], 'overlayColor', {sliders[1][2],sliders[2][2],sliders[3][2],sliders[4][2]})
		setShaderFloatArray(shit[i], 'satinColor', {sliders[5][2],sliders[6][2],sliders[7][2],sliders[8][2]})
		setShaderFloatArray(shit[i], 'innerShadowColor', {sliders[9][2],sliders[10][2],sliders[11][2],sliders[12][2]})
		setShaderFloat(shit[i], 'innerShadowAngle', sliders[13][2]*(math.pi/180))
		setShaderFloat(shit[i], 'innerShadowDistance', sliders[14][2])
	end
end
