...
function onStartCountdown()
	if not seenCutscene and enabled then
		...
		elseif cutsceneData.hasDialogue and not viewedDialogue then -- In case of Dialogue
			...
			viewedDialogue = true
			return Function_Stop
		elseif instructionsData.hasMechanic and difficulty >= instructionsData.diff and not showMechanic and not botPlay then -- If there are mechanics
			showMechanic = true
...
function onUpdate(elapsed)
	...
	if instructionsData.hasMechanic and difficulty >= instructionsData.diff and showMechanic then
        ...
        if pressAction() and not confirmMechanic then
