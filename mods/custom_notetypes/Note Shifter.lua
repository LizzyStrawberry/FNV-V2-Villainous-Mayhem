...
function onUpdate()
	if mechanics then
		...
		if healthDraining == true and getPropertyFromClass('ClientPrefs', 'buff3Active') == false then 
			if curStep % 1 == 0 then
				if framerate <= 240 and framerate > 120 then --For some reason in higher fps, the drain becomes faster :what the fuck:
				...
				elseif framerate == 120 then
					if getPropertyFromClass('ClientPrefs', 'resistanceCharm') == 1 then
						if isMayhemMode then
							setProperty('health', health - 0.0165)
						else
							setProperty('health', health - 0.00165)
						end
					else
						if isMayhemMode then
				...
				elseif framerate <= 59 and framerate > 30 then --For some reason in lower fps, the drain becomes slower, cuz yk, based onUpdate()
					if getPropertyFromClass('ClientPrefs', 'resistanceCharm') == 1 then
						if isMayhemMode then
							setProperty('health', health - 0.0465)
						else
							setProperty('health', health - 0.00465)
						end
					else
						if isMayhemMode then
							setProperty('health', health - 0.097)
						else
