...
function onUpdate()
	...
	if difficulty == 1 and healthDraining == true and getPropertyFromClass('ClientPrefs', 'buff3Active') == false and songName == "Get Villain'd" then 
		if curStep % 1 == 0 then
		...
		if difficulty == 1 and healthDraining == true and getPropertyFromClass('ClientPrefs', 'buff3Active') == false then
			if curStep % 1 == 0 then
				...
				elseif framerate <= 59 and framerate > 30 then --For some reason in lower fps, the drain becomes slower :what the fuck:
					if not isMayhemMode then
						if getPropertyFromClass('ClientPrefs', 'resistanceCharm') == 1 then		
							setProperty('health', health - 000135)
							healthMinus = false
						else
							setProperty('health', health - 0.0027)
							healthMinus = false
...
function noteMiss(id, noteData, noteType, isSustainNote)
	if noteType == 'Real Poison' then
		--debugPrint('Timer Started!')
		cameraFlash('game', '4c9e64', 0.5, false)

		if difficulty == 1 and songName == "Get Villain'd" then
