...
function onStartCountdown() -- for dialogue
	...
	if mechanics and difficulty >= 0 and allowMechanic then
...
function onNextDialogue(line)
	if line == 11 then
		playDialogueVideo('Week2_Song4Cutscene');
		triggerEvent('Change Character', 'dad', 'Yaku')
	end
end
...
function onCreate()
	if mechanics and difficulty >= 0 then
		...
		makeLuaSprite('Warning', 'instructions/Inst-Yaku', 0, 0)
