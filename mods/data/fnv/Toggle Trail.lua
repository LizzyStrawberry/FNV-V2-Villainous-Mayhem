...
function createTrailFrame(tag)
	...
	if not (image == '') then
		...
		if tag == 'Dad' then
			setProperty(trailTag..'.angle', getProperty('dad.angle'));
		elseif tag == 'BF' then
			setProperty(trailTag..'.angle', getProperty('boyfriend.angle'));
		end
